...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
    ...
    const processFile = (file: File) => {
        if (file.size > 100 * 1024 * 1024) {
            console.log("File too large (max 100MB)");
            return;
        }
        setFiles([file]);
        if (isImageFile(file)) {
                        ...
                        {files.map((file, index) => (
                            <div key={index} className="relative group">
                                {file.type.startsWith("image/") && filePreviews[file.name] ? (
                                    ...
                                    <div className="flex items-center gap-2 rounded-xl bg-[var(--color-bg)] px-3 py-2 border border-[var(--color-border)] text-sm">
                                        <span className="truncate max-w-[120px] text-gray-400">{file.name}</span>
                                        <button
                                            onClick={() => handleRemoveFile(index)}
                                            className="rounded p-0.5 hover:bg-[#3A3A40]"
                                        >
                                            <X className="h-3 w-3 text-white" />
                                        </button>
                                    </div>
